 ...
 * - Connection status indication (green=connected, blue blinking=waiting, off=sleeping)
 * - Battery status indication (green=good, yellow=low, red blinking=critical, blue=charging)
...
void indicator_set_batt_state(batt_state_t state) {
  if (indicator_get_batt_state() == state) return;
